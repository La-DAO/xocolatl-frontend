...
export function computeUserHealthRatio(
  reserveBal: BigNumber,
  mintedCoinBal: BigNumber,
  collatRatioNum: BigNumber,
  collatRatioDenom: BigNumber,
  price: BigNumber,
  liqParamGlobalBase: BigNumber,
) {
  ...
  console.log(
    "DEBUG liqParamGlobalBase: ",
    utils.formatEther(liqParamGlobalBase),
  );

  const reserveBalReducedByFactor: BigNumber = reserveBal
    .mul(collatRatioDenom)
    .div(collatRatioNum);

  const maxMintableAmount = reserveBalReducedByFactor
    .mul(price)
    .div(BigNumber.from(liqParamGlobalBase));
